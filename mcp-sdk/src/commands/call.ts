...
/** Shared options threaded through batch/session flows. */
...
/**
 * Provider-signed receipt returned per call in batch/session mode.
...
/**
 * Core call loop shared by batch and session flows.
...
async function runChannelCallLoop(
  ...
  while (!shouldStop()) {
    ...
    if (response.status === 402) {
      console.log(
        `  Status: 402 — channel may not be active yet. Stopping loop.`,
      );
      callCount--;
      break;
    }
...
async function runBatchApiCallsFlow(opts: ChannelFlowOpts) {
  const { client, url, token, maxCalls, ratePerCallOverride, depositOverride } =
    opts;

  console.log(`  Batch mode: up to ${maxCalls} calls per channel`);
  console.log("");
  ...
  // ratePerCall = user override OR probe price.
  // IMPORTANT: if this provider has multiple endpoints with different prices,
  // set --rate-per-call to the HIGHEST price any endpoint may charge.
  // The contract enforces cumulativeCost <= sequenceNumber * ratePerCall.
  const ratePerCall = ratePerCallOverride ?? BigInt(offer.maxAmountRequired);
  const deposit = depositOverride ?? ratePerCall * BigInt(maxCalls);

  console.log(`  Provider:      ${offer.payTo}`);
  console.log(
    `  Rate/call cap: ${formatUnits(ratePerCall, token?.decimals || 18)} ${token?.symbol}`,
  );
  ...
  console.log(
    `  Total deposit: ${formatUnits(deposit, token?.decimals || 18)} ${token?.symbol}`,
  );
  console.log("");
  ...
  const { txHash: openTxHash, channelId } = await client.openChannel({
    ...
    deposit,
    maxSpend: deposit,
    maxDuration: 24 * 60 * 60, // 24h window for batch sessions
    ratePerCall,
  });
  ...
  // called in a different context (e.g. an SDK user wrapping this client).
  client.setChannelForProvider(offer.payTo, channelId);
  console.log(`  Interceptor notified of channel.`);
  console.log("");

  // Step 3: wait for provider activation (off-chain handshake)
  ...
  if (activated) {
    ...
    console.log(
      "  Provider did not activate within 90 s. Proceeding anyway — " +
        "the first call will carry the channel intent.",
    );
  }
  console.log("");

  // Step 4: call loop (bounded by maxCalls)
  let callsMade = 0;
...
async function runSessionedCallsFlow(opts: ChannelFlowOpts) {
  const {
    client,
    url,
    token,
  ...
  console.log(
    `  Session mode: ${durationSecs}s window, up to ${maxCalls} calls`,
  );
  console.log("");
  ...
  console.log(`  Session:       ${durationSecs}s`);
  console.log(`  Max calls:     ${maxCalls}`);
  console.log(
  ...
  // Step 2: open channel with the session duration as expiry
  console.log("  Opening payment channel on-chain...");
  const { txHash: openTxHash, channelId } = await client.openChannel({
    provider: offer.payTo,
    token: offer.asset,
  ...
  const sessionDeadline = Date.now() + durationSecs * 1000;
  let callsMade = 0;
  const { callCount, lastReceipt } = await runChannelCallLoop(
    opts,
    channelId,
    ...
    () => {
      callsMade++;
      return Date.now() >= sessionDeadline || callsMade > maxCalls;
    },
  );
