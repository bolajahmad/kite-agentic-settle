import { createSignedReceipt, verifyReceipt, validateReceipt, } from "./receipt.js";
export class ChannelManager {
    ...
    receipts = new Map();
    constructor(contractService, token, privateKey, signerAddress) {
    ...
    async openChannel(config) {
        ...
        const result = await this.contractService.openChannel(config.provider, token, mode, config.deposit, config.maxDuration, config.ratePerCall);
        if (!result.channelId) {
    ...
    async closeChannel(channelId) {
        ...
        return await this.contractService.closeChannel(channelId, lastReceipt.nonce, lastReceipt.cumulativeCost, lastReceipt.timestamp, lastReceipt.signature);
    }
