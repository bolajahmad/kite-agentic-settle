...
// ── Tool Handler ───────────────────────────────────────────────────
export async function handleTool(client, toolName, args) {
    switch (toolName) {
        case "call_paid_api": {
            const url = args.url;
            ...
            const maxAmount = args.maxAmount ? BigInt(args.maxAmount) : undefined;
            const fetchOpts = {};
            if (!autopay)
                fetchOpts.autoPayEnabled = false;
            if (maxAmount)
                fetchOpts.maxPaymentPerCall = maxAmount;
            const init = { method };
        ...
        case "check_balance": {
            const balance = await client.getTokenBalance(args.token);
            return {
